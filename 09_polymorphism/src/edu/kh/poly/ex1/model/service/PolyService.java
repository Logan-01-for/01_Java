...
public class PolyService {
	...
	public void ex1() {
		...
		// The method is undefined for the type Car
		...
		// -> car2,car3는 부모 타입 참조변수 (Car)이기 때문에
		...
		// 다형성을 이용한 객체 배열 :
		// 부모 타입 참조 자료형의변수를 하나의 묶음으로 다루는 것
		...
		// 다형성(업캐스팅) : 부모타입 참조 변수 arr[i]로 자식 객체를 참조할 수 있다.
			
	}
	
	// 전달 받은 Car 또는 자식 객체 (Tesla, Spark)의 엔진, 연료, 바퀴갯수를 출력하는 메서드
	public void printCar(Car temp) {
		...
		// 부모 타입 참조변수 = 자식 타입 객체
		//-> 다형성의 업캐스팅 모양과 똑같다
		// -> temp 라는 Car 타입 매개변수에는 Car, Tesla, Spark의 주소가 참조되어도 된다
	...
	// 매개변수로 전달 받은 정수 값에 따라 Car, Tesla, Spark 객체를 만들어
	// 반환하는 메서드
	...
	public void ex3() {
		...
		// 주의 !			// 다운캐스팅
		// "." 연산자 (Tesla) 형변환하는 연산자보다 우선 순위 높음
		...
		public void ex4() {
			...
			Spark s1 = (Spark)c1; // c1을 관계 없는 Spark로 다운 캐스팅 하려고 함
			// 다운 캐스팅 잘못해도 컴파일 에러 발생 안함
			//-> 실행 시 발생 (런타임 에러)
			// ClassCastException : 형변환 예외
			
			// 해결 방법 : instanceof 와 같이 사용하면 된다!
			
			// 예외 처리
			if ( c1 instanceof Spark ) { // c1이 Spark 이거나 상속 관계인가?
				// 맞다면
		...
		public void ex5() {
			...
			// edu.kh.poly.ex1.dto.Car.toString()
			// 참조변수 c2가 Car 타입이므로
			// toString() 호출 코드와 Car의 toString() 메소드 코드를 연결
			//-> 정적 바인딩
			
			// 실행해보면 자식(Spark)의 toString() 이 호출되는 것을 확인
			// 경차 엔진 / 휘발유 / 4 / 0.5
			//-> 컴파일 시에는 부모 (Car)와 바인딩 == 정적 바인딩
			//-> "실행 시" 에는 정적 바인딩보다 자식(spark)에서 오버 라이딩 된
