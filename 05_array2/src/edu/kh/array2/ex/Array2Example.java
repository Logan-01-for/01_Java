...
public class Array2Example { }
	...
	public void ex1() {
		...
		*/
		...
		// * 배열 길이
		System.out.println(arr.length); // = 2 / (행길이)length 앞
		...
		// Arrays.toSring(배열명) : 참조하고 있는 1차원 배열의 값을 문자열로 반환
...
for(int col = 0; col < arr[0].length; col++) { // 각 열을 순회
			...
			for(int col = 0; col < arr.length; col++) { // 각 행을 순회
	...
	public void ex3() {
		...
		char[][] arr = new char[4][];// for 첫 결과
		// char 2차원 배열 생성 시 행 부분만 생성
		
		arr[0] = new char[3]; // 0 행에 3열 짜리 1차원 배열을 생성하여 주소값 저장
		...
		char ch= 'a';
		/*
		for (int fow = 0; row < arr.length; row++) { // 4
			for(int col = 0; col < arr[row].length; col++) {
				
				arr[row][col] = ch++; // 후위 연산
