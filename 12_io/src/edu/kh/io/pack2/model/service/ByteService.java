...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			// String 의 불변성 문제를 해결한 객체 StringBuilder(비동기)/ StringBufffer(동기)
			...
			// 출력 방법 1 : 한 글자(2byte = char)씩 파일로 출력
			...
			*/
			...
			// ******* 수행 시간 확인 *********
			// write() 수행 되는 시간 측정
			// System.nanoTime() : 1970.01.01 오전 9:00 부터
			...
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 102000ns
			
			
			fos.flush(); // 스티림 안에 남아있는 모든 데이터를 모두 밀어냄
			// FileOutStream에서는 flush() 선택 사항
		...
		} finally { // try 에외 발생 여부 관계 없이 무조건 수행
			// 무조건 수행 
			...
			try {
				// 스티림이 정상 생성된 경우
				// 스티림을 닫는다(NullPointerException 방지)
	...
	/**
	 * 버퍼를 이용한 파일 출력
	 * + BufferedOutputStream 보조 스티림 이용
	 * 
	 * */
	public void bufferedFileByteOutput() {
		 ...
		 * 출력용 보조 스티림(바이트) / 입력용 보조 스트림(바이트)
		 * 
		 * - FileOurputStream 은 1 바이트씩 데이터를 입출력
		 * -> 통로가 좁아서 데이터 입출력시 속도가 느림
		 * 
		 * -> 보조스트림 이용 시 버퍼에 입출력할 내용을 모아서 한번에 입출력!
		 * (내부 버퍼에 데이터를 모아 뒀다가 일정 크기가 되면 한 번에 출력)
		...
		try {
			...
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 44200ns
			...
			try {
				...
				// 보조 스트림 (bos) close()시;
				// 보조스티림 생성에 사용된 기반 스트림 (fos)도 같이 close() 됨
	...
	public void fileByteInput() {
		...
		try {
			...
			// -> 존재하지 않으면 FileNoeFoundException 발생
			...
			while(true) {
				...
				sb.append((char) value ); // char로 강제 형 변환해서 글자 형태로 sb 추가
	...
	try {
		fis = new FileInputStream("\\io_test\\2025\\노래가사.txt");
	 ...
	 * 
	 * InputStream 의 경우 외부 파일의 데이터를 읽어올때 사용
	...
	public void readProperties() {
		 ...
		 * 
		 * [실행화면]
		 * 파일 경로 입력 : /io_tes/20250305/노래가사.txt
		 * 복사 완료 : /io_test/20250305/노래가사_ copy.txt
		 * 
		...
		*/
		public void fileCopy() {
			...
			try {
				...
				// 출력 할 파일의 경로 + _copy 가 붇은 파일 이름
				// target : /io_test/20240923/노래가사.txt 
				// copy 	: /io_test/20240923/노래가사.txt
				...
				try {
					// 사용한 스트림 메모리 모두 반환
					if(br !=null)br.close();
					if(br !=null)bis.close(); //fis
					if(br !=null)bos.close();// fos
