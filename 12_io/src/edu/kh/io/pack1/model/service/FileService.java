...
public class FileService {
	...
	public void method1 () {
		...
		if( !directory.exists()) { // 폴더가 존재하지 않으면
			...
			System.out.println(directory.getName()); // 이름
			System.out.println(directory.getPath()); // 반환
		}
	...
	public void method2() {
		...
		if( !file.exists()) { // 존재하지 않으면
			...
			// 파일 생성
			try{
				// File 클래스의 매서드는 대부분 IOException 발생 시킴
				// -> IOException는 반드시 예외처리를 해야 하느 ㄴChecked Exception
				// -> 예외 처리 필수!
	...
	public void method3() {
		...
		// File 객체 생성
		File dirctory = new File("\\worspace\\01_Java\\12_IO");
		
		// 지정된 경로에 있는 모든 파일/디렉토리를 File[] 형태로 얻어오기
		File[] files = dirctory.listFiles(); // 디렉토리 내 파일 목록을 File[] 배열 형태로 반환
		...
		for(File f : files) {
			...
			// java.text.SimpledDateFormat : 간단히 날짜 형식을 지정할 수있는 객체
