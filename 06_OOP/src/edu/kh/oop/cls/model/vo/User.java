...
public class User {
	...
	// 매개변수 생성자 자동완성
	// alt + shit + s -> Generate Constructor using Fields.. -> generate
	
	// ** 매개변수 : 생성자나 메서드 호출 시 () 안에 작성되어
	//		전달되어지는 값을 저장하고 있는 변수
	// -> 전달 받은 값ㅇ르 저장하고 있는 매개체 역할의 변수
	...
	// 매개 변수의 개수와 동일한것이 있으나, 하나의 타입이 다름
	// -> 오버로딩 성립
	...
	// 1. 만약에 오타가 있거나 , 부모 클래스에 없는 메서드를 잘못 작성 했을 경우 컴파일 오류 발생
	// 2. 이 메서드가 부모 클래스를 재정의 했다는 것을 한눈에 알 수 있음

@Override// @(어노테이션) : 컴파일러 인식용 주석
public String toString() {
	
	return "User [userId=" + userId + ", userPw=\" + userPw + \", userName=\" + userName + \", userAge=\" + userAge + \", userGender=" +userGender +"]";
}


private String userAge(String string, char userGender2) {
	...
	// 객체가 문자열로 변환 될 때 호출 되는 메서드
	// -> 보통 객체의 필드 값을 출력하는 용도로 오버 라이딩해서 사용함.
	
	
	/*
	 * 생성자 (constructor)
	 * 
	 * - new 연산자를 통해서 객체를 생성 할때
