...
public class Run {
	...
	public static void main(String[] args) {
		// 3명의 학생 정보를 기록할 수있게 객체 배열 할당
		Student[] students = new Student[3];// import 해야됨
		
		// 위의 사용데이터 참고하여 3명의 학생 정보 초기화
		students[0] = new Student("홍길동", 20, 178.2, 70.0, 1, "정보시스템공학과");
		students[0] = new Student("김말똥", 21, 187.3, 80.0, 2, "경영학과");
		students[0] = new Student("강개순", 23, 167.0, 45.0, 4, "정보xhdtls공학과");
		
		// 반복문을 통해 출력
		// for / while..
		// 향상된 for문
		/*
		 * 자바에서 배열, 컬랙션과 같은 데이터를 순차적으로 간단하게 반복할 수있는
		 * 구조를 제공
		...
		for( Student std: students) {// 순차 접근하면서 접근
			System.out.println(std.information() );
		}
		
		System.out.println("------------------------------------");
		
		// 최대 10명의 사원 정보를 기록할 수있게 객체 배열을 할당
		...
		// 추가 여부 묻기
			...
			/*
			if (str2.equalsIgnoreCase("N")) break; : 문자열 비교 메서드, 대소문자를 구분하지않고
			// 두 문자열이 같은지 비교
		*
		*
		*/
		...
		for (Employee emp : emps) {// 10칸 모두 순회
			// [첫번째 EMP 객체에 대한 주소, 두번째 EMP객체 주소, null, null, null, null, null, null] 
